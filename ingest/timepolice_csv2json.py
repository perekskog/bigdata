"""Convert one CSV file generated by TimePolice app.
"""
import csv
import sys
import json
    ...
    with open(file, 'r', encoding=input_encoding) as f:
        ...
        for row in reader:
            ...
            if len(row) >= skipcolumns+2 and row[skipcolumns+1] == "...":
                ...
                taskname = ""
                continue
            if len(row) == 1+skipcolumns or (len(row) >= 2+skipcolumns
                                             and row[1+skipcolumns] == ""):
                if sessionname == "":
                ...
                if row[0+skipcolumns] == "":
                    ...
                    if stoptime < starttime:
                        ...
                        stoptime = session_time(sessioncreated, session_day_offset,
                                                row[1+skipcolumns])
                    ...
                    if taskname == "":
                        ...
                        starttime = session_time(sessioncreated, session_day_offset,
                                                 row[1+skipcolumns])
...
def main(filein, input_encoding, input_delimiter, base_date, skipcolumns, fileout):
    if input_delimiter == "tab":
        items = fetch_items(filein, input_encoding, '\t',
                            datetime.strptime(base_date, "%y-%m-%d"), int(skipcolumns))
    else:
        items = fetch_items(filein, input_encoding, input_delimiter,
                            datetime.strptime(base_date, "%y-%m-%d"), int(skipcolumns))

    open(fileout, 'wb').write(json.dumps(items, sort_keys=True, indent=4,
                                         default=date_handler, ensure_ascii=False).encode('utf8'))
